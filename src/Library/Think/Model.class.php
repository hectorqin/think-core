 ...
 *
 * @method static strict($option)
 * @method static order($option) 排序
 * @method static alias($option) 设置model别名
 * @method static having($option) having筛选条件
 * @method static group($option) group
 * @method static lock($option) 是否锁表
 * @method static distinct($option) distinct
 * @method static auto($option) 自动完成
 * @method static filter($option) 过滤
 * @method static validate($option) 校验
 * @method static result($option) 数据编码方式 json/xml
 * @method static token($option) 自动表单令牌验证
 * @method static index($option) 对数据集进行索引
 * @method static force($option)
 * @method int count($field = null) 统计数量，参数是要统计的字段名（可选）
 * @method int|float sum($field) 获取总分，参数是要统计的字段名（必须）
 * @method int|float max($field) 获取最大值，参数是要统计的字段名（必须）
 * @method int|float min($field) 获取最小值，参数是要统计的字段名（必须）
 * @method int|float avg($field) 获取平均值，参数是要统计的字段名（必须）
 */
class Model
{
    // 操作状态
    ...
    public function __construct($name = '', $tablePrefix = '', $connection = '')
    {
        ...
        if (!empty($name)) {
            if (strpos($name, '.')) {
                ...
                $this->name = $name;
                //THINK_CHANGE_BEGIN  自动添加数据库名
                $this->dbName = parse_db_name($this->name, true);
                //THINK_CHANGE_END
            }
     ...
     * @return static
     */
    public static function instance($name = '', $tablePrefix = '', $connection = '')
    {
    ...
    protected function _before_insert(&$data, $options)
    {
        ...
        Hook::listen($this->name . '_before_insert', $params);
        Hook::listen('_before_insert', $params);
    }
    ...
    {
        $params = [$this, $data, $options];
        Hook::listen($this->name . '_after_insert', $params);
        Hook::listen('_after_insert', $params);
    }
    ...
    {
        $params = [$this, $data, $options];
        Hook::listen($this->name . '_before_update', $params);
        Hook::listen('_before_update', $params);
    }
    // 更新成功后的回调方法
    protected function _after_update($data, $options)
    {
        $params = [$this, $data, $options];
        Hook::listen($this->name . '_after_update', $params);
        Hook::listen('_after_update', $params);
    }
    ...
    {
        $params = [$this, null, $options];
        Hook::listen($this->name . '_before_delete', $params);
        Hook::listen('_before_delete', $params);
    }
    // 删除成功后的回调方法
    protected function _after_delete($data, $options)
    {
        $params = [$this, $data, $options];
        Hook::listen($this->name . '_after_delete', $params);
        Hook::listen('_after_delete', $params);
    }
    ...
    protected function _after_select(&$resultSet, $options)
    {
        if (!empty($options['with'])) {
            $this->getRelations($resultSet, $options['with']);
        }
    }

    /**
     * 查询数据集，多次返回指定条记录，不支持缓存，不支持select主键参数以及返回sql
     * @param  array  $options [description]
     * @return array          [description]
     */
    public function selectMany($options = array())
    {
        // 分析表达式
        $options = $this->_parseOptions($options);
        if (!isset($options['page'])) {
            $options['page'] = array(1, 20);
        } else {
            $options['page'][0] = (int) $options['page'][0] > 0 ? (int) $options['page'][0] : 1;
        }
        while ($resultSet = $this->db->select($options)) {
            ++$options['page'][0];
            yield $resultSet;
        }
    }
    ...
    /**
     * 分块处理
     * @param int $chunkNum 分块数量
     * @param callable $callback 回调函数
     * @return false|void
     * @throws Exception
     */
    public function chunk($chunkNum, $callback)
    {
        if (is_null($callback)) {
            return false;
        }
        ...
        $options         = $this->_parseOptions($options);
        $options['page'] = array(1, $chunkNum);
        while ($resultSet = $this->db->select($options)) {
            ++$options['page'][0];
            $resultSet = array_map(array($this, '_read_data'), $resultSet);
            $this->_after_select($resultSet, $options);
            $result = \call_user_func_array($callback, [$resultSet]);
            if ($result === false) {
                break;
            }
        }
    ...
    /**
     * 查询数据集并返回PDOStatement
     * @param callable $callback
     * @param array $options
     * @return mixed
     * @throws Exception
     */
    public function cursor($callback = null, $options = array())
    {
        $options['cursor'] = true;
        /** @var \PDOStatement $PDOStatement */
        $PDOStatement = $this->select($options);
        if (\is_null($callback)) {
            return $PDOStatement;
        } else {
            while ($row = $PDOStatement->fetch()) {
                $resultSet = array_map(array($this, '_read_data'), [$row]);
                $this->_after_select($resultSet, $options);
                $result = \call_user_func_array($callback, $resultSet);
                if ($result === false) {
                    break;
    ...
    protected function _parseType(&$data, $key)
    {
        if (!isset($this->options['bind'][':' . $key]) && isset($this->fields['_type'][$key])) {
            ...
            } elseif (C("DB_FIELD_LENGTH_ADAPT")) {
                // 数据库字段长度自动裁剪
                preg_match('/(char|varchar|text)\s*\((\d+)\)/', $fieldType, $match);
                // 匹配到存在长度限制的 char|varchar|text 类型
                if (isset($match[2]) && (int) ($match[2]) > 0) {
                    $data[$key] = mb_substr($data[$key], 0, (int) $match[2], "utf-8");
                }
            }
        }
    }
     ...
     * @access public
     * @param mixed $join
     * @param string $type JOIN类型 THINK_CHANGE  将默认join类型改为left join
     * @return Model
     */
    ...
    /**
     * 返回 PDOStatement, 仅查询有效
     * @access public
     * @param boolean $isReturn 是否返回 PDOStatement
     * @return Model
     */
    public function returnPDO($isReturn = true)
    {
        $this->options['cursor'] = $isReturn;
        return $this;
    }
    ...
    {
        return $this->db->getProperty('PDOStatement');
    }
    ...
    public function saveAll($datas)
    {
        ...
        foreach ($datas as $data) {
            foreach ($data as $key => $value) {
                ...
                if ($pk === $key) {
                    ...
                    $lists[$key] .= sprintf("WHEN %u THEN '%s' ", $data[$pk], addslashes($value));
                }
            }
        }
        ...
        $options = array($pk => array('in', $ids));
        if (false === $this->_before_update($datas, $options)) {
            return false;
        }
        $result = $this->execute($sql);
        if (false !== $result && is_numeric($result)) {
            $this->_after_update($datas, $options);
        }
        return $result;
    }
     ...
     * @param mixed $relations 关联模型
     * @return void
     */
    protected function getRelations(&$resultSet, $relations)
    {
        ...
        foreach ((array) $relations as $name => $relation) {
            if (\is_int($name) && is_string($relation)) {
                ...
                if (\strpos($name, '.') === false) {
                    ...
                    if (is_array($relation)) {
                        // 合并选项 'a' => []
                        $relation = \array_merge($defaultRelations[$name] ?? [], $relation);
                    } else if ($relation instanceof \Closure) {
                        // 设置回调函数 'a' => function(){}
                        $relation = \array_merge($defaultRelations[$name] ?? [], [
                            'query_callback' => $relation,
                        ]);
                    }
                } else {
                    // 'a.b' => [] 子关联模型
                    $subRelation  = $relation;
                    $subRelations = \explode('.', $name);
                    $relationName = \array_shift($subRelations);
                    $relation     = $defaultRelations[$relationName] ?? false;

                    if ($relation) {
                        $relation['with'] = [
                            \implode('.', $subRelations) => $subRelation,
                        ];
                    }
                }
            }
            ...
            if (!isset($relation['relation_model'])) {
                // 没有设置model就去掉子关联模型
                unset($relation['with']);
            }
            $resultSet = \loadRelation($resultSet, $relation);
        }
    }
}
