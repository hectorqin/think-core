...
class Think
{
    ...
    {
        if ($e = error_get_last()) {
            Log::record(json_encode($e, JSON_UNESCAPED_UNICODE), Log::EMERG);
            Log::record(json_encode(debug_backtrace(), JSON_UNESCAPED_UNICODE), Log::EMERG);
            Log::save();
            switch ($e['type']) {
                case E_ERROR:
                case E_PARSE:
