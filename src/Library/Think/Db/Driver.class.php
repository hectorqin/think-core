...
abstract class Driver
{
    ...
    {
        // THINK_CHANGE_BEGIN 判断mysql连接是否需要重连
        if (isset($this->linkID[$linkNum]) && $this->config['type'] == 'mysql' && C('MYSQL_AUTO_RECONNECT')) {
            try {
                $this->linkID[$linkNum]->getAttribute(PDO::ATTR_SERVER_INFO);
            } catch (\PDOException $e) {
                if (strpos($e->getMessage(), 'MySQL server has gone away') !== false) {
                    \Think\Log::record(L('_MYSQL_AUTO_RECONNECT_'));
     ...
     * @param boolean $returnPDOStatement  返回PDOStatement
     * @param boolean $master  是否在主服务器读操作
     * @return mixed
     */
    public function query($str, $fetchSql = false, $returnPDOStatement = false, $master = false)
    {
        ...
        try {
            ...
            if (false === $result) {
                ...
                return $returnPDOStatement ? $this->PDOStatement : $this->getResult();
            }
        } catch (\PDOException $e) {
            $this->error();
            return false;
     ...
     * @return mixed
     */
    public function execute($str, $fetchSql = false, $returnPDOStatement = false)
    {
        ...
        try {
            ...
            if (false === $result) {
                ...
                return $returnPDOStatement ? $this->PDOStatement : $this->numRows;
            }
        } catch (\PDOException $e) {
            $this->error();
            return false;
            ...
            if (is_string($val[0])) {
                ...
                if (preg_match('/^(eq|neq|gt|egt|lt|elt)$/', $exp)) {
                    ...
                    if (isset($val[2]) && 'exp' == $val[2]) {
                        ...
                        if (!$zone) {
                            Log::record("IN/NOT IN 查询，范围数组不能为空", Log::WARN);
                        }
                        $whereStr .= $key . ' ' . $this->exp[$exp] . ' (' . ($zone ? $zone : '""') . ')';
                    }
    ...
    public function select($options = array())
    {
        ...
        $this->parseBind(!empty($options['bind']) ? $options['bind'] : array());
        $sql    = $this->buildSelectSql($options);
        $result = $this->query($sql, !empty($options['fetch_sql']) ? true : false, !empty($options['cursor']) ? $options['cursor'] : false, !empty($options['master']) ? true : false);
        return $result;
    }
