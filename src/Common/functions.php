...
function C($name = null, $value = null, $default = null)
{
    ...
    if (is_string($name)) {
        ...
        //THINK_CHANGE_BEGIN  二维/三维数组设置和获取支持
        $name    = explode('.', $name);
        $name[0] = strtoupper($name[0]);
        if (is_null($value)) {
            return isset($name[2]) ? (isset($_config[$name[0]][$name[1]][$name[2]]) ? $_config[$name[0]][$name[1]][$name[2]] : $default) : (isset($_config[$name[0]][$name[1]]) ? $_config[$name[0]][$name[1]] : $default);
        } else {
            if (isset($name[2])) {
        ...
        //THINK_CHANGE_END  二维/三维数组设置和获取支持
        /*        // 二维数组设置和获取支持
    $name = explode('.', $name);
    $name[0]   =  strtoupper($name[0]);
    if (is_null($value))
...
function parse_db_name(&$string, $returnDB = false)
{
    if (empty($string)) {
        return $string;
    }
    if (C('DB_AUTO_PACK') != null) {
        $crossDb = C('DB_AUTO_PACK');
        if ($returnDB) {
            if (strpos($string, '_')) { //设置了跨库配置且有表前缀
                list($prefix) = explode('_', $string);
                $prefix       = $prefix . '_';
                if (isset($crossDb[$prefix])) {
                    list($dbName) = explode('.', $crossDb[$prefix]);
                    return $dbName;
                }
            }
            return '';
        }
        $regex  = "/[\ ](" . implode('|', array_keys($crossDb)) . ")/sU";
        $string = preg_replace_callback($regex, function ($match) use ($crossDb) {return ' ' . $crossDb[$match[1]];}, $string = ' ' . $string);
        $string = substr($string, 1);
/*        foreach ($crossDb as $key => $value) {
$string = preg_replace_callback("/\ $key/sU",function ($match) use($value) {return strtolower($value);},' '.$string);
}*/
    }
    if ($returnDB) {
        return '';
...
function loadRelation($data, $relation)
{
    if (!$data) {
        return $data;
    }
    $linkKeys     = array_unique(array_column($data, $relation['foreign_key']));
    $relationList = [];
    $hasMany      = false;
    if ($linkKeys) {
        $where = [
            $relation['relation_table'] . '.' . $relation['relation_key'] => ['in', $linkKeys],
        ];
        if (isset($relation['condition'])) {
            $where = array_merge($where, isset($relation['condition']) ? $relation['condition'] : []);
        }
        if (isset($relation['relation_model'])) {
            $modelClass = $relation['relation_model'];
            $query      = $modelClass::instance();
            if (isset($relation['with'])) {
                $query = $query->with($relation['with']);
            }
        } else {
            $query = M($relation['relation_table'], null);
        }
        if (isset($relation['mapping_limit'])) {
            $query = $query->limit($relation['mapping_limit']);
        }
        if (isset($relation['mapping_order'])) {
            $query = $query->order($relation['mapping_order']);
        }
        if (isset($relation['mapping_group'])) {
            $query = $query->group($relation['mapping_group']);
        }
        if (isset($relation['mapping_join'])) {
            foreach ($relation['mapping_join'] as $join) {
                $query = $query->join($join);
            }
        }
        if (isset($relation['mapping_fields'])) {
            $query = $query->field($relation['mapping_fields']);
        }
        if (isset($relation['query_callback']) && $relation['query_callback']) {
            $result = call_user_func_array($relation['query_callback'], [$query]);
            $query  = $result ?: $query;
        }
        if (($alias = $query->option('alias'))) {
            $map = [];
            foreach ($where as $key => $value) {
                $key       = str_replace($relation['relation_table'] . '.', $alias . '.', $key);
                $map[$key] = $value;
            }
            $where = $map;
        }
        $result = $query->where($where)->select();
        $result = $result ? $result : [];

        if (isset($relation['mapping_type']) && ($relation['mapping_type'] === \Think\Model\RelationModel::HAS_MANY || $relation['mapping_type'] === \Think\Model\RelationModel::MANY_TO_MANY)) {
            foreach ($result as $item) {
                $relationList[$item[$relation['relation_key']]][] = $item;
            }
            $hasMany = true;
        } else {
            $relationList = array_column($result, null, $relation['relation_key']);
        }
    }
    foreach ($data as &$item) {
        $relationData = isset($relationList[$item[$relation['foreign_key']]]) ? $relationList[$item[$relation['foreign_key']]] : null;
        if (isset($relation['mapping_name'])) {
            $item[$relation['mapping_name']] = $relationData;
        }
        if ($relationData && !$hasMany) {
            if (isset($relation['as_fields']) && $relation['as_fields']) {
                if ($relation['as_fields'] === true) {
                    $item = array_merge($item, $relationData);
                } else {
                    foreach ((array) $relation['as_fields'] as $field) {
                        $item[$field] = $relationData[$field];
                    }
                }
            }
        }
